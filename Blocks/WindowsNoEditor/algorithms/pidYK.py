class PIDController_x:
    ...
    # Bound integral term to avoid windup
    def clamp_integral(self, integral_value):
        return max(    min(integral_value, self.windup_limit),  -self.windup_limit    )

    def saturate(self, output):
    # Bound output to a reasonable range
        return max(     min(output, 10.0), -10.0    )  # Adjust saturation
...
class PIDController_z:
    ...
    def update(self, current_value, dt):
        ...
        self.integral += error * dt
        derivative = (error - self.prev_error) / dt
        output = self.Kp * error + self.Ki * self.integral + self.Kd * derivative + 9.8
        self.prev_error = error
        g=9.8
        return output
...
try:
    ...
    while True:
        ...
        dt_actual = time.time() - last_time
        last_time = time.time()
        
        
        # Compute control signal using PID controller
        control_signal = [
        pid_controller_x.update(current_pos[0], dt_actual),
        pid_controller_y.update(current_pos[1], dt_actual),
        pid_controller_z.update(current_pos[2], dt_actual)
        ]
